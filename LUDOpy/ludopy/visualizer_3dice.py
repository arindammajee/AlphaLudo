...
def draw_dice_list(board, dice_list, player):
    if len(dice_list)==2:
        text = f"{dice_list[0]} {dice_list[1]}"
    elif len(dice_list)==1:
        text = f"{dice_list[0]}"
    else:
        text = ""
    _, _, _, bot_right, _ = get_taile_cord(5, 17)
    draw_multi_box(board, (7, 17), (4, 18), line_color=(0, 0, 0))
    draw_multi_box(board, (7, 17), (4, 18), fill_color=(255, 255, 255))
    
    draw_text(board, "{}".format(text), bot_right, PLAYER_COLORS[player], fontScale=2, thickness=2)

def calculate_points(piece_positions):
        points = 0
        for i in range(4):
...
def draw_points(board, all_pieces):
    points_list = []
    draw_multi_box(board, (9, 17), (13, 18), line_color=(0, 0, 0))
    draw_multi_box(board, (9, 17), (13, 18), fill_color=(255, 255, 255))

    _, _, _, bot_right, _ = get_taile_cord(7, 17)
    draw_text(board, "{}".format(f"Points"), bot_right, (0, 0, 0), fontScale=1, thickness=2)

    for i in range(4):
        point = calculate_points(all_pieces[i])
        points_list.append(point)
        _, _, _, bot_right, _ = get_taile_cord(9+i, 17)
        draw_text(board, "{}".format(f"P{i}: {point}"), bot_right, PLAYER_COLORS[i], fontScale=1, thickness=2)
...
def draw_moment(board, moment):
    pieces, dice_list, dice, players_dice, round_i = moment
    ...
    if dice != -1:
        ...
        draw_dice_list(board, dice_list, players_dice)
        draw_points(board, pieces)
...
def make_img_of_board(pieces, dice_list, dice, players_dice, round_number):
    board = draw_basic_board()
    draw_moment(board, (pieces, dice_list, dice, players_dice, round_number))
    return board
