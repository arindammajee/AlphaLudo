...
class Game:
    ...
    def __dice_generator(self):
        """
        Will update self.dice_list with a 3 new random numbers on the dice
        """
        self.dice_list = [np.random.randint(1, 6 + 1),np.random.randint(1, 6 + 1), np.random.randint(1, 6 + 1)]
    
    def _dice_choose(self):
        """
    ...
    def __add_to_hist(self):
        ...
        self.hist.append([pieces, self.dice_list, self.current_dice, self.current_player, self.round])
    ...
    def get_observation(self):
        ...
        if len(self.dice_list)==0:
            ...
            obs = self.__gen_observation(self.current_player, roll_dice=False)
        #print(self.dice_list)
    ...
    def answer_observation(self, piece_to_move):
        ...
        after_obs = self.__gen_observation(self.current_player, roll_dice=False, choose_dice=False)
    ...
    def points_given(self):
        ...
        points_list = []
        for i in range(4):
            points_list.append(self.calculate_points(all_pieces[i]))
